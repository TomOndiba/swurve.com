...
<div id="panel">
    ...
    <div id="allphotos" style="width: 810px; height: 170px; margin-top: -15px; background-color: #fff; border: 0;" class="jThumbnailScroller">
        ...
        <?php $current_photo = 0; $count = 0; foreach(ORM::factory('user', array('username' => $username))->photos->where('approved', '<>' , 'No')->and_where('hide', '=', 'No')->find_all() as $pho): $count++; ?>
            <?= HTML::anchor('nphoto/' . $username . '/' . $pho->uniqueid, HTML::image(Content::factory($username)->get_photo($pho->uniqueid, 'l'), $photo == $pho->uniqueid ?  array('style' => 'border: 3px solid #28619C !important;') : null )); ?>
