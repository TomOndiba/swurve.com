...
<?= Form::label('signup_date_after', 'On or After Signup Date:'); ?> <?= Form::input('signup_date_after', $post['signup_date_after']); ?> <br />
...
echo Form::label('startid', 'Start From User ID:'); 
echo Form::input('startid', $post['startid']) . '<br />';
/*
...
*/
?>
<br /><h3>Select a flirt from the list below:</h3>
<?php foreach($messages as $message): ?>
<div>
    <div style="float: left; width: 30px; margin-top: 4px;"><?= Form::radio('message', $message->id, (isset($post['message']) AND $post['message'] == $message->id) ? TRUE : FALSE, array('class' => 'message')); ?></div>
    <div style="float: left; display: block; width: 580px; font-size: 10px; line-height: 15px; margin-bottom: 10px;">
        <?= $message->message; ?>
    </div>
</div>
<br /><br />
